...
RzBitVector *M68KQemuFPRegisterToRizin(const RzBitVector *qemu_value) {
	...
	switch (rz_bv_len(qemu_value)) {
	case 64:
		return RzBitVectorBinary80FromBinary64(qemu_value);
	case 96:
		return RzBitVectorBinary80FromM68KExtended96(qemu_value);
	default:
		return nullptr;
	}
}

RzBitVector *M68KRizinFPRegisterToQemu(const RzBitVector *rizin_value, size_t qemu_bits) {
	if (!rizin_value || rz_bv_len(rizin_value) != 80) {
		return nullptr;
	}
	switch (qemu_bits) {
	case 64:
		return RzBitVectorBinary64FromBinary80(rizin_value);
	case 96:
		return RzBitVectorM68KExtended96FromBinary80(rizin_value);
	default:
		return nullptr;
	}
}
...
bool M68KRizinFloatsEquivalent(const RzBitVector *expected, const RzBitVector *actual, size_t qemu_bits) {
	...
	RzBitVector *expected_qemu = M68KRizinFPRegisterToQemu(expected, qemu_bits);
	RzBitVector *actual_qemu = M68KRizinFPRegisterToQemu(actual, qemu_bits);
...
class M68KTraceAdapter : public TraceAdapter {
	public:
		...
		void CustomRegSetup(RzReg *rz_reg, const std::string &trace_reg_name, const RzBitVector *trace_bv) const override {
			...
			RzBitVector *rizin_value = valid_width ? M68KQemuFPRegisterToRizin(trace_bv) : nullptr;
			if (!item || !rizin_value) {
		...
		bool CustomRegCompare(RzReg *rz_reg, const std::string &trace_reg_name,
			...
			const bool valid_width = trace_bits == 96 ||
				(trace_bits == 64 && GetMachine() == frame_mach_mcf_isa_b_float_emac);
			RzBitVector *expected_rizin = valid_width ? M68KQemuFPRegisterToRizin(trace_bv) : nullptr;
			RzBitVector *expected_qemu = M68KRizinFPRegisterToQemu(expected_rizin, trace_bits);
			RzBitVector *actual_rizin = item ? rz_reg_get_bv(rz_reg, item) : nullptr;
			RzBitVector *actual_qemu = M68KRizinFPRegisterToQemu(actual_rizin, trace_bits);
